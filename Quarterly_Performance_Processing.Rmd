...
# Referrals Tables -------------------------------------------------------------
monthly_referral_vol <- readRDS("C:/Users/kweons01/Desktop/Strategic Initiatives - So Youn/Github/quarterly-processing/monthly_referral_vol.rds")

# Volume Tables ----------------------------------------------------------------
## Arrived monthly volume ======================================================
monthly_vol <- readRDS("C:/Users/kweons01/Desktop/Strategic Initiatives - So Youn/Github/quarterly-processing/monthly_vol.rds")
## Arrived Epic department quarterly volume ====================================
vol_zip_code <- readRDS("C:/Users/kweons01/Desktop/Strategic Initiatives - So Youn/Github/quarterly-processing/vol_zip_code.rds")

# Access Tables ----------------------------------------------------------------
## Wait Time Horizon by Specialty ==============================================
waitTime_dist_specialty <- readRDS("C:/Users/kweons01/Desktop/Strategic Initiatives - So Youn/Github/quarterly-processing/waitTime_dist_specialty.rds")
waitTime_dist_site <- readRDS("C:/Users/kweons01/Desktop/Strategic Initiatives - So Youn/Github/quarterly-processing/waitTime_dist_site.rds")
                                   
## Median Wait Time ============================================================
waitTime_mshs <- readRDS("C:/Users/kweons01/Desktop/Strategic Initiatives - So Youn/Github/quarterly-processing/waitTime_mshs.rds")
waitTime_site <- readRDS("C:/Users/kweons01/Desktop/Strategic Initiatives - So Youn/Github/quarterly-processing/waitTime_site.rds")
waitTime_prov <- readRDS("C:/Users/kweons01/Desktop/Strategic Initiatives - So Youn/Github/quarterly-processing/waitTime_prov.rds")

# Payor Mix Table --------------------------------------------------------------
payer_mix_quarter_site <- readRDS("C:/Users/kweons01/Desktop/Strategic Initiatives - So Youn/Github/quarterly-processing/payer_mix_quarter_site.rds")
...
quarters_list <- sort(unique(as.yearqtr(seq(from=as.Date("2021-01-01") , to=floor_date(Sys.Date() - months(1), "month"), by="month"))))
...
waitTime_prov <- merge(waitTime_prov, productivity_raw[,c("NPI", paste0(reporting_year, " Calculated %"))])
...
# waitTime_prov$productivity <- productivity_raw$`2022 Calculated %`[match(waitTime_prov$NPI, productivity_raw$NPI)]
...
deptSite_monthly_vol_function_1_sub <- function(department){
...
deptSite_monthly_vol_function_2_sub <- function(department){
  # 
  # department <- "Otolaryngology"
...
dept_access_takeaway <- function(department){
  ...
  dept_med_waitTime <- as.character(unique(as.numeric((wait_time_new %>% filter(New.PT3 == "New"))$med_wait)))
...
productivity_scatter_function <- function(department){
  ...
  waitTime_prod_output <- waitTime_prov %>%
    filter(Campus.Specialty.Group == department) %>%
    filter(Appt.Made.YearQtr %in% reporting_YearQtr) %>%
    mutate(productivity = round(.[[7]],2)*100) %>%
...
productivity_scatter_function_sub <- function(department){
...
productivity_table_function_sub <- function(department){
...
productivity_provider_50_function <- function(department){
  ...
  productivity_tbl <- productivity_tbl %>%
    mutate(last_year = abs(round(.[[length(productivity_tbl)-1]],2))*100,
           current_year = abs(round(.[[length(productivity_tbl)]],2))*100) %>%
    arrange(current_year) %>%
    dplyr::select(-c(12:13))
  
  productivity_tbl$waitTime <- waitTime_prov$med_wait[match(productivity_tbl$NPI, waitTime_prov$NPI)]
...
productivity_provider_50_function_sub <- function(department){
...
productivity_provider_75_function_sub <- function(department){
...
productivity_provider_100_function_sub <- function(department){
...
dept_payer_mix <- function(department){
  ...
  payer_mix <- payer_mix_quarter_site %>%
    filter(Campus.Specialty.Group == department) %>%
    filter(Appt.YearQtr %in% reporting_YearQtr) %>%
    group_by(Campus.Specialty.Group, Campus, Appt.YearQtr, Coverage) %>%
    summarise(payer = sum(payer)) %>%
    group_by(Campus) %>%
    mutate(total_vol = sum(payer),
           payer_perc = round(payer/sum(payer),2)*100) %>%
    dplyr::select(-c(payer, Campus.Specialty.Group, Appt.YearQtr)) %>%
 ...
 tbl_output <- 
   ...
   width(j = 1, width = 1.5) %>%
   width(j = 2, width = 1) %>%
   width(j = c(3:length(payer_mix)), width = 1.3) %>%
   height(height = .4, part = "body")
   # rotate(j = c(3:length(payer_mix)), rotation = "lrtb", align = "bottom", part = "header")
...
dept_payer_mix_sub <- function(department){
  ...
  payer_mix <- payer_mix_quarter_site %>%
    ...
    mutate(total_vol = sum(payer),
           payer_perc = round(payer/sum(payer),2)*100) %>%
    dplyr::select(-c(payer, Campus.Specialty.Group, Campus.Specialty.SubGroup, Appt.YearQtr)) %>%
    pivot_wider(names_from = Coverage,
                values_from = payer_perc,
                values_fill = 0) %>%
    rename(Site = Campus,
...
# department <- "Otolaryngology"
  ...
  newPTs_mshs <- waitTime_dist_specialty %>%
    ...
    filter(Appt.Made.YearQtr %in% reporting_YearQtr) %>%
    filter(New.PT3 == "New") %>% 
    group_by(Campus.Specialty.Group, Appt.Made.YearQtr, New.PT3, group) %>%
    summarise(total = sum(total)) %>%
  ...
  newPTs_site <- waitTime_dist_site %>%
    ...
    filter(Appt.Made.YearQtr %in% reporting_YearQtr) %>%
    filter(New.PT3 == "New") %>% 
    group_by(Campus.Specialty.Group, Campus, Appt.Made.YearQtr, New.PT3, group) %>%
    summarise(total = sum(total)) %>%
    mutate(new_perc = round(total/sum(total),2)) %>%
    filter(group %in% c("0-7","8-14")) %>%
    group_by(Campus.Specialty.Group, Campus, Appt.Made.YearQtr) %>%
    summarise(new_perc = sum(new_perc)*100) %>%
    pivot_wider(names_from = Campus,
  ...
  conversion_rate_all$Metric <- "Conversion"
    ...
    # % Physicians <P75
    P75_mshs <- waitTime_prov %>%
      filter(Campus.Specialty.Group == department) %>%
      filter(Appt.Made.YearQtr %in% reporting_YearQtr) %>%
      mutate(group = ifelse(`2022 Calculated %` >=.75, "Yes","No")) %>%
      group_by(Campus.Specialty.Group, Appt.Made.YearQtr, group) %>%
      summarise(total = n()) %>%
      group_by(Campus.Specialty.Group, Appt.Made.YearQtr) %>%
      mutate(Overall = (total/sum(total))*100) %>%
      filter(group == "No") %>%
      dplyr::select(Campus.Specialty.Group, Appt.Made.YearQtr, Overall) 

    P75_site <- waitTime_prov %>%
      filter(Campus %!in% c("MSDD","NETWORK")) %>%
      filter(Campus.Specialty.Group == department) %>%
      filter(Appt.Made.YearQtr %in% reporting_YearQtr) %>%
      mutate(group = ifelse(`2022 Calculated %` >=.75, "Yes","No")) %>%
      group_by(Campus.Specialty.Group, Appt.Made.YearQtr, Campus, group) %>%
      summarise(total = n()) %>%
      group_by(Campus.Specialty.Group, Appt.Made.YearQtr, Campus) %>%
      mutate(Overall = (total/sum(total))*100) %>%
      filter(group == "No") %>%
      dplyr::select(Campus.Specialty.Group, Appt.Made.YearQtr, Campus, Overall) %>%
      pivot_wider(names_from = Campus,
                  values_from = Overall)
    ...
    metrics_merged <- bind_rows(waitTime_overall, newPTs_overall, vol_var_overall, conversion_rate_all, p75_overall)
    metrics_merged <- metrics_merged %>%
      dplyr::select(-c(Campus.Specialty.SubGroup, Quarter)) %>%
      rename(`MSH-Amb Care` = `MSH- AMBULATORY CARE`)
  ...
  tbl_output <- 
    ...
    hline(i = c(4,5,6), border = fp_border(color="grey", width = 2), part="body") %>%
    fix_border_issues() %>%
...
department <- j
  ...
  newPTs_mshs <- waitTime_dist_specialty %>%
    ...
    filter(Appt.Made.YearQtr %in% reporting_YearQtr) %>%
    filter(New.PT3 == "New") %>% 
    group_by(Campus.Specialty.SubGroup, Appt.Made.YearQtr, New.PT3, group) %>%
    summarise(total = sum(total)) %>%
    mutate(new_perc = round(total/sum(total),2)) %>%
    filter(group %in% c("0-7","8-14")) %>%
    group_by(Campus.Specialty.SubGroup, Appt.Made.YearQtr) %>%
    summarise(new_perc = sum(new_perc)*100)
  ...
  newPTs_site <- waitTime_dist_site %>%
    ...
    filter(Appt.Made.YearQtr %in% reporting_YearQtr) %>%
    filter(New.PT3 == "New") %>% 
    group_by(Campus.Specialty.SubGroup, Campus, Appt.Made.YearQtr, New.PT3, group) %>%
    summarise(total = sum(total)) %>%
    mutate(new_perc = round(total/sum(total),2)) %>%
    filter(group %in% c("0-7","8-14")) %>%
    group_by(Campus.Specialty.SubGroup, Campus, Appt.Made.YearQtr) %>%
    summarise(new_perc = sum(new_perc)*100) %>%
    pivot_wider(names_from = Campus,
                values_from = new_perc)
  ...
  vol_var_mshs <- monthly_vol %>%
      ...
      pivot_longer(5:6,
                   names_to = "Metric",
                   values_to = "Overall") %>%
      dplyr::select(Campus.Specialty.SubGroup, Metric, Overall)
    ...
    vol_var_site <- monthly_vol %>%
      ...
      filter(Appt.YearQtr %in% c(data_quarters[length(data_quarters)], data_quarters[length(data_quarters)-1], data_quarters[length(data_quarters)-4])) %>%
      mutate(Appt.YearQtr = as.character(Appt.YearQtr)) %>%
      group_by(Campus.Specialty.SubGroup, Campus, Appt.YearQtr) %>%
      summarise(total = sum(monthly_vol)) %>%
      pivot_wider(names_from = Appt.YearQtr,
                  values_from = total) 
    ...
    vol_var_site <- vol_var_site %>%
      dplyr::select(Campus.Specialty.SubGroup, Campus, var_last_qrt, var_last_yr) %>%
      pivot_longer(c("var_last_qrt","var_last_yr"),
                   names_to = "Metric",
                   values_to = "Values") %>%
      pivot_wider(names_from = "Campus",
  ...
  conversion_rate_all$Metric <- "Conversion"
    ...
    P75_mshs <- waitTime_prov %>%
      ...
      filter(Appt.Made.YearQtr %in% reporting_YearQtr) %>%
      mutate(group = ifelse(`2022 Calculated %` >=.75, "Yes","No")) %>%
      group_by(Campus.Specialty.SubGroup, Appt.Made.YearQtr, group) %>%
      summarise(total = n()) %>%
      group_by(Campus.Specialty.SubGroup, Appt.Made.YearQtr) %>%
      mutate(Overall = (total/sum(total))*100) %>%
      filter(group == "No") %>%
      dplyr::select(Campus.Specialty.SubGroup, Appt.Made.YearQtr, Overall) 
    ...
    P75_site <- waitTime_prov %>%
      ...
      filter(Appt.Made.YearQtr %in% reporting_YearQtr) %>%
      mutate(group = ifelse(`2022 Calculated %` >=.75, "Yes","No")) %>%
      group_by(Campus.Specialty.SubGroup, Appt.Made.YearQtr, Campus, group) %>%
      summarise(total = n()) %>%
      group_by(Campus.Specialty.SubGroup, Appt.Made.YearQtr, Campus) %>%
      mutate(Overall = (total/sum(total))*100) %>%
      filter(group == "No") %>%
      dplyr::select(Campus.Specialty.SubGroup, Appt.Made.YearQtr, Campus, Overall) %>%
      pivot_wider(names_from = Campus,
                  values_from = Overall)
    ...
    metrics_merged <- bind_rows(waitTime_overall, newPTs_overall, vol_var_overall, conversion_rate_all, p75_overall)
    metrics_merged <- metrics_merged %>%
      dplyr::select(-c(Campus.Specialty.Group, Campus.Specialty.SubGroup, Quarter))
...
specialties <- as.vector(unique(monthly_vol[,c("Campus.Specialty.Group", "Campus.Specialty.SubGroup")]))
specialty_group <- unique(specialties$Campus.Specialty.Group)
medicine_group <- unique((specialties %>% filter(Campus.Specialty.Group == "Medicine"))$Campus.Specialty.SubGroup)



for(i in specialty_group){
  ...
  ppt_temp <-  ph_with(ppt_temp, value = paste0("Medicine - ",j), 
                       location = ph_location_label(ph_label = "Text Placeholder 6"))
    
      # 1.1 Additional Section Header for Medicine ---------------------------------
  ## Section Header 
  # ppt_temp <- ppt_temp %>%
  ...
  ppt_temp <- ph_with(ppt_temp, value = paste0("Volume - ",j), 
                      location = ph_location_label(ph_label = "Title 1"))
  ppt_temp <- ph_with(ppt_temp, value = dept_monthly_vol_function_sub(j),
  ...
  ### Key Takeaways 
  vol_slide_takeaways <- block_list(fpar(ftext(paste0("Total volume for ",reporting_YearQtr," was ",dept_monthly_vol_takeaway_sub(j)[2],
                                                      " (",dept_monthly_vol_takeaway_sub(j)[1]," was in-person visits)"),
                                               prop = fp_text(font.family = 'Calibri',font.size=16, bold = TRUE))),
                                    fpar(ftext(paste0("Average monthly volume for this quarter was ",dept_monthly_vol_takeaway_sub(j)[3]),
                                               prop = fp_text(font.family = 'Calibri',font.size=14))),
                                    fpar(ftext(paste0("This is ",dept_monthly_vol_takeaway_sub(j)[4]," change from last quarter"),
                                               prop = fp_text(font.family = 'Calibri',font.size=14))),
                                    fpar(ftext(paste0("This is ",dept_monthly_vol_takeaway_sub(j)[5]," change from same quarter last year"),
                                               prop = fp_text(font.family = 'Calibri',font.size=14))),
                                    fpar(ftext(paste0(dept_monthly_vol_takeaway_sub(j)[6]," saw the largest volume with ",dept_monthly_vol_takeaway_sub(j)[7],
                                                      " visits (",dept_monthly_vol_takeaway_sub(j)[8]," of total volume)"),
                                               prop = fp_text(font.family = 'Calibri',font.size=14))))
  ...
  ppt_temp <- ph_with(ppt_temp, value = paste0("Volume Trending - ",j), 
                      location = ph_location_label(ph_label = "Title 1"))
  ppt_temp <- ph_with(ppt_temp, value = deptSite_monthly_vol_function_1_sub(j),
                      location = ph_location_label(ph_label = "Chart Placeholder 6"))
  ppt_temp <- ph_with(ppt_temp, value = deptSite_monthly_vol_function_2_sub(j),
                      location = ph_location_label(ph_label = "Table Placeholder 9"))
  text_style <- fp_text(font.size = 10, italic = TRUE, color = "#7f7f7f", font.family = "Calibri")
  par_style <- fp_par(text.align = "left", padding.bottom = 5)
  ppt_temp <- ph_with(ppt_temp, value = fpar(ftext("Includes Epic data only; metrics do not account for any data prior to Epic go-live.",
  ...
  ppt_temp <- ph_with(ppt_temp, value = paste0("Access - ",j), 
                      location = ph_location_label(ph_label = "Title 1"))
  ppt_temp <- ph_with(ppt_temp, value = overall_time_to_appt_dist_function_sub(j),
                      location = ph_location_label(ph_label = "Chart Placeholder 6"))
  ppt_temp <- ph_with(ppt_temp, value = site_time_to_appt_function_sub(j),
                      location = ph_location_label(ph_label = "Table Placeholder 9"))
  text_style <- fp_text(font.size = 10, italic = TRUE, color = "#7f7f7f", font.family = "Calibri")
  par_style <- fp_par(text.align = "left", padding.bottom = 5)
  ppt_temp <- ph_with(ppt_temp, value = fpar(ftext("Includes Epic data only; metrics do not account for any data prior to Epic go-live.",
  ...
  ### Key Takeaways 
  access_slide_takeaways <- block_list(fpar(ftext(paste0("Total new patient visits scheduled in ",reporting_YearQtr," was ",dept_access_takeaway_sub(j)[1],
                                                         " (",dept_access_takeaway_sub(j)[2]," of total visits scheduled)"),
                                                  prop = fp_text(font.family = 'Calibri',font.size=16, bold = TRUE))),
                                       fpar(ftext(paste0(dept_access_takeaway_sub(j)[3],
                                                         " of new in-office ",j,
                                                         " patients are seen within 14 days of scheduling the appointment (target: <=14 days)"),
                                                  prop = fp_text(font.family = 'Calibri',font.size=14))),
                                       fpar(ftext(paste0(dept_access_takeaway_sub(j)[4],
                                                         " of new telehealth ",j,
                                                         " patient are seen within 14 days of scheduling the appointment (target: <=14 days)"),
                                                  prop = fp_text(font.family = 'Calibri',font.size=14))),
                                       fpar(ftext(paste0("Median time between scheduling and an actual appointment for new ",j," patients across all sites is ",
                                                         dept_access_takeaway_sub(j)[5], " days"),
                                                  prop = fp_text(font.family = 'Calibri',font.size=14))))
  ...
  ppt_temp <- ph_with(ppt_temp, value = paste0("Referral Volume - ",j), 
                      location = ph_location_label(ph_label = "Title 1"))
  ppt_temp <- ph_with(ppt_temp, value = conversion_rate_function_sub(j),
                      location = ph_location_label(ph_label = "Chart Placeholder 6"))
  text_style <- fp_text(font.size = 10, italic = TRUE, color = "#7f7f7f", font.family = "Calibri")
  par_style <- fp_par(text.align = "left", padding.bottom = 5)
  ppt_temp <- ph_with(ppt_temp, value = fpar(ftext("Includes Epic data only; metrics do not account for any data prior to Epic go-live; Conversion Rate is percent of scheduled appointments over recerrals received; Completion Rate is percent of arrived appointments over referrals received.",
                                                   prop = text_style), fp_p = par_style),
                      location = ph_location_label(ph_label = "Footer Placeholder 4"))
  ...
  # ppt_temp <- ph_with(ppt_temp, value = paste0("Physician Productivity Overview - ",j), 
  #                     location = ph_location_label(ph_label = "Title 1"))
  # ppt_temp <- ph_with(ppt_temp, value = productivity_scatter_function(j),
  #                     location = ph_location_label(ph_label = "Chart Placeholder 6"))
  ...
  #                     location = ph_location_label(ph_label = "Text Placeholder 7"))
  # ppt_Temp <- ph_with(ppt_temp, value = productivity_table_function(j),
  #                     location = ph_location_label(ph_label = "Table Placeholder 9"))
  
  ### Productivity by Provider 
  ppt_temp <- ppt_temp %>%
    add_slide(layout = "Title and Content_Wide", master = "Default Theme")
  ppt_temp <- ph_with(ppt_temp, value = paste0("Physician Productivity <50% - ",j), 
                      location = ph_location_label(ph_label = "Title 1"))
  ppt_temp <- ph_with(ppt_temp, value = productivity_provider_50_function_sub(j),
                      location = ph_location_label(ph_label = "Content Placeholder 2"))
  
  ppt_temp <- ppt_temp %>%
    add_slide(layout = "Title and Content_Wide", master = "Default Theme")
  ppt_temp <- ph_with(ppt_temp, value = paste0("Physician Productivity 50-74% - ",j), 
                      location = ph_location_label(ph_label = "Title 1"))
  ppt_temp <- ph_with(ppt_temp, value = productivity_provider_75_function_sub(j),
                      location = ph_location_label(ph_label = "Content Placeholder 2"))
  
  ppt_temp <- ppt_temp %>%
    add_slide(layout = "Title and Content_Wide", master = "Default Theme")
  ppt_temp <- ph_with(ppt_temp, value = paste0("Physician Productivity >=75% - ",j), 
                      location = ph_location_label(ph_label = "Title 1"))
  ppt_temp <- ph_with(ppt_temp, value = productivity_provider_100_function_sub(j),
                      location = ph_location_label(ph_label = "Content Placeholder 2"))
  ...
  # ppt_temp <- ph_with(ppt_temp, value = paste0("Overall Utilization - ",j),
  #                     location = ph_location_label(ph_label = "Title 1"))
  # ppt_temp <- ph_with(ppt_temp, value = overall_day_util_tbl_function(j),
  #                     location = ph_location_label(ph_label = "Chart Placeholder 6"))
  ...
  # ppt_temp <- ppt_temp %>%
  #   add_slide(layout = "Two Vertical Tables", master = "Default Theme")
  # ppt_temp <- ph_with(ppt_temp, value = paste0("Utilization by Hour - ",j), 
  #                     location = ph_location_label(ph_label = "Title 1"))
  # ppt_Temp <- ph_with(ppt_temp, value = overall_weekday_util_hour(j),
  #                     location = ph_location_label(ph_label = "Chart Placeholder 6"))
  # ppt_Temp <- ph_with(ppt_temp, value = site_weekday_util_hour(j),
  #                     location = ph_location_label(ph_label = "Table Placeholder 9"))
  ...
  ppt_temp <- ph_with(ppt_temp, value = paste0("Payer Mix - ",j), 
                      location = ph_location_label(ph_label = "Title 1"))
  ppt_temp <- ph_with(ppt_temp, value = dept_payer_mix_sub(j),
                      location = ph_location_label(ph_label = "Chart Placeholder 6"))
  text_style <- fp_text(font.size = 10, italic = TRUE, color = "#7f7f7f", font.family = "Calibri")
  par_style <- fp_par(text.align = "left", padding.bottom = 5)
  ppt_temp <- ph_with(ppt_temp, value = fpar(ftext("Includes Epic data only; metrics do not account for any data prior to Epic go-live.",
  ...
  ppt_temp <- ppt_temp %>% 
    ph_with(value = paste0(paste0(reporting_YearQtr," ", "Metrics Overview"," - ",j)), 
            location = ph_location_label(ph_label = "Title 1")) 
  ppt_temp <- ph_with(ppt_temp, value = metrics_overview_tbl_sub(j),
                      location = ph_location_label(ph_label = "Content Placeholder 2"))
  text_style <- fp_text(font.size = 10, italic = TRUE, color = "#7f7f7f", font.family = "Calibri")
  par_style <- fp_par(text.align = "left", padding.bottom = 5)
  ppt_temp <- ph_with(ppt_temp, value = fpar(ftext("Includes Epic data only; metrics do not account for any data prior to Epic go-live.",
  ...
  ppt_temp <- ppt_temp %>% 
    ph_with(value = paste0("Action Plan"," - ",j), 
            location = ph_location_label(ph_label = "Title 1")) 
  ...
  print(ppt_temp, target=paste0("Medicine - ",gsub("/", "_", j),"_Performance Report_",reporting_YearQtr,".pptx"))
  ...
  ppt_temp <- ppt_temp %>%
    add_slide(layout = "Action_Plan_Slide", master = "Default Theme")
  ppt_temp <- ppt_temp %>% 
    ph_with(value = paste0("Action Plan"," - ",i), 
            location = ph_location_label(ph_label = "Title 1")) 
  ppt_Temp <- ph_with(ppt_temp, value = actionPlan_output,
                      location = ph_location_label(ph_label = "Content Placeholder 8"))
 ...
 ppt_temp <- ppt_temp %>%
  ...
  # Compiled Slide Output --------------------------------------------------------
  print(ppt_temp, target=paste0(gsub("/", "_", j),"_Performance Report_",reporting_YearQtr,".pptx"))
